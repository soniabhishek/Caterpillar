...
class ProxyResource:
    ...
    def __loadFromProxyServer(self, proxies, url):
        ...
        if results is not None:
            for x in results:
                ...
                port = result[1]
                print
                "length:%s ip:%s  port:%s " % (len(result), ip, port)
                model = ProxyModel(ip, port, "proxyServer")
                proxies.append(model)
                count += 1
        print
        "---proxyLoader---:load proxy from proxyServer get %s " % count
    ...
    def __loadProxyFromURL(self, proxies, url, pattern):
        ...
        print
        "---proxyloader---:load proxy from %s" % url
        source = self.html_getter.getHtmlRetry(url, 3)
        ...
        if results is not None:
            for result in results:
                ...
                count += 1
        print
        "---proxyloader---:load proxy from %s (get %s)" % (url, count)
    ...
    def saveToFile(self, file_abspath, proxies):
        ...
        f.close()
        print
        "$proxy/> write proxies to %s." % f.name
...
class WebPageDownloader:
    ...
    def getHtmlRetry(self, url, retry=0):
        ...
        while retry > 0:
            ...
            if error_msg is not None:
                print
                "[X] HtmlGetter err:%s, retry:%s." % (error_msg, retry_count)

            if error_msg is None and self.validate_html(html):
                print
                "[v] success access webpage."
                return source
    ...
    def validate_html(self, html):
        ...
        if html.code in HTTPErrors:
            print
            "error found: %s: %s " % HTTPErrors[html.code]
            return False
        else:
            return True;
